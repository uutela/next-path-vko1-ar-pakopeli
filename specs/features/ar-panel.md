...
**Then** they are exactly `src/ui/ArScreen.tsx` and `src/ui/PuzzlePanel.tsx`, and `src/ui/ArScreen.web.tsx` imports neither of them
...
### AC18: The web AR screen says where the puzzle can be opened
**Given** the `ArScreen` resolved for web, rendered with `PUZZLE_STATE`
**When** it is rendered
**Then** exactly one node reads `Tehtävä avataan puhelimen sovelluksessa.`, and no keypad key is present
...
| `src/ui/ArScreen.web.tsx` | New. Web stand-in that imports no Viro |
| `src/ui/ArScreen.web.test.tsx` | New. AC17 and AC18 |
...
| source tree | happy path | `ArScreen.web.tsx` | imports inspected | imports neither Viro nor `PuzzlePanel` (AC17) |
| `ArScreen` (web) | happy path | `PUZZLE_STATE` | rendered | one `Tehtävä avataan puhelimen sovelluksessa.`, no keys (AC18) |
